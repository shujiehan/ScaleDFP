...
class Memory:
    ...
    def __init__(self, path, cur_date, positive_window_size, model, columns, features, \
        ...
        self.sum_time_buffering = 0
        self.sum_time_labeling = 0
        self.sum_time_filtering = 0
    ...
    def buffering(self):
        for i in range(1, self.positive_window_size):
            ...
            if len(df_failed.index) > 0:
                self.df = self.labeling(df_failed['serial_number'].values, None)
                self.sum_time_labeling += (time.time() - t1)
            t1 = time.time()
            self.df = pd.concat([self.df, df_delta])
            self.sum_time_buffering += (time.time() - t1)
        ...
        self.sum_time_filtering += (time.time() - t1)
