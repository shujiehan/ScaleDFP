...
public class OzaBagAdwin extends AbstractClassifier implements MultiClassClassifier {
    ...
    public FloatOption deltaOption = new FloatOption("deltaOption", 'd', "The delta for ADWIN", 0.002, 0.0, 1.0);
    private ExecutorService executor;

    public int changeDetectedEnsemble = 0;
    public int changeDetectedBaseLearner = 0;
    ...
    public void resetLearningImpl() {
        ...
        for (int i = 0; i < this.ensemble.length; i++) {
            this.ADError[i] = new ADWIN(deltaOption.getValue());
        }

        this.changeDetectedEnsemble = 0;
        this.changeDetectedBaseLearner = 0;
        // Shujie Add:
        // Multi-threading
        int numberOfJobs;
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        Collection<DetectingRunnable> detectors = new ArrayList<DetectingRunnable>();
        for (int i = 0; i < this.ensemble.length; i++) {
            //Shujie Add:
            int k;
            // Shujie ScaleDFP: bypass the customized Poisson sampling in StreamDFP
            ...
            //}
            k = inst.getPoissonWeights().get(i);
            if (k > 0) {
        ...
        //Shujie: taking the following loop out of training loop for multi-threading
        for (int i = 0; i <  this.ensemble.length; i++) {
            if (this.executor != null) {
                DetectingRunnable detector = new DetectingRunnable(this.ensemble[i], this.ADError[i], inst);
                ...
                if (this.ADError[i].setInput(correctlyClassifies ? 0 : 1)) {
                    if (this.ADError[i].getEstimation() > ErrEstim) {
                        Change = true;
                        this.changeDetectedBaseLearner++;
                    }
                }
            }
        }
        if(this.executor != null) {
            try {
                List<Future<Boolean>> retChanges = this.executor.invokeAll(detectors);
                for (Future<Boolean> retChange: retChanges) {
                    if (retChange.get()) {
                        Change = true;
                        this.changeDetectedBaseLearner++;
                    }
        ...
        if (Change) {
            ...
            if (imax != -1) {
                ...
                this.ADError[imax] = new ADWIN(deltaOption.getValue());
            }
        }
    }
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        ...
        measurementList.add(new Measurement("ensemble size", this.ensemble != null ? this.ensemble.length : 0));
        measurementList.add(new Measurement("Change detected base learner", this.changeDetectedBaseLearner));
        measurementList.add(new Measurement("Change detected ensemble", this.changeDetectedEnsemble));
        this.changeDetectedBaseLearner = 0;
        this.changeDetectedEnsemble = 0;
    ...
    protected class DetectingRunnable implements Runnable, Callable<Boolean> {
        final private Classifier learner;
        final private Instance instance;
        final private ADWIN ADError;
        private Boolean Change;

        public DetectingRunnable(Classifier learner, ADWIN ADError, Instance inst) {
            this.learner = learner;
        ...
        @Override
        public void run() {
            boolean correctlyClassifies = learner.correctlyClassifies(this.instance);
