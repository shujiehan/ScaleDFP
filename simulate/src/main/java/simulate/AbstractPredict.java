...
public abstract class AbstractPredict extends AbstractOptionHandler {
    ...
    // Shujie added on May 27, 2024
    public int indexPoissonWeight;
    public int numPoissonWeight;
    // record serializable serial numbers
    protected List<String> recordSN = new ArrayList<>();
    ...
    public void updateModel(Instances trainingData, AbstractClassifier scheme, double downSampleRatio) {
        ...
        // get indices of Poisson weights in attributes
        Attribute poissonWeightStart = trainingData.attribute("weight_" + indexPoissonWeight);
        int startIdxPoissonWeight = trainingData.indexOf(poissonWeightStart);
        recordSN.clear();
        // Note: updated downsampling ratio would not be used in Bagging and ARF, but used in Boosting
        for (int i = 0; i < trainingData.numInstances(); i++) {
            ...
            String sn = attrSN.get((int)inst.value(idxSerialNumber));
            recordSN.add(sn);
            // record Poisson weights into each instance
            int k = 0;
            List<Integer> poissonWeights = new ArrayList<>();
            for (int j = indexPoissonWeight; j < indexPoissonWeight + numPoissonWeight; j++) {
                poissonWeights.add((int)inst.value(startIdxPoissonWeight + k));
                k++;
            }
            inst.setPoissonWeights(poissonWeights);
        }

        // delete poisson weights in attributes
        // need to delete the attribute of Poisson weight from the last one due to the implementation of deleteAttributeAt()
        for (int j = startIdxPoissonWeight + numPoissonWeight - 1; j >= startIdxPoissonWeight; j--) {
            trainingData.deleteAttributeAt(j);
        }
